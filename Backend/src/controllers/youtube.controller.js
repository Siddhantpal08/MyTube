...
const searchVideos = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        const response = await axios.get(searchApiUrl, { timeout: 10000 }); // 10-second timeout

        const nextPageToken = response.data.nextPageToken;

        // Simplify the response to send only what the frontend needs
        const simplifiedVideos = response.data.items.map(item => ({
        ...
        return res.status(200).json(
            new ApiResponse(200, responseData, "YouTube search results fetched successfully")
        );
