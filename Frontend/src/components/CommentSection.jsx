...
const CommentCard = ({ comment, onCommentDeleted, onCommentUpdated }) => {
    ...
    // A comment is only editable/deletable if it's NOT an external YouTube comment and the user is the owner.
    const isOwner = !comment.isExternal && (user?._id === comment.owner?._id);

    const handleDelete = async () => {
        try {
            await axiosClient.delete(`/comments/${comment._id}`);
    ...
    const handleUpdate = async (e) => {
        e.preventDefault();
        try {
            const response = await axiosClient.patch(`/comments/${comment._id}`, { content: editedContent });
    ...
    return (
        <>
            ...
            <div className="flex items-start space-x-4">
                <img src={comment.owner?.avatar || placeholderAvatar} alt={comment.owner?.username} className="w-10 h-10 rounded-full object-cover bg-gray-700" />
