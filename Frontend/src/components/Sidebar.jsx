...
const NavItem = ({ to, icon, text, isOpen, end = false }) => (
    <NavLink 
        ...
        className={({ isActive }) => 
            `flex items-center p-4 rounded-lg transition-colors ${isActive ? 'bg-gray-700 text-white' : 'text-gray-400 hover:bg-gray-800 hover:text-white'}`
        }
        title={text}
    >
        {icon}
        <span className={`ml-4 font-medium transition-opacity duration-200 ${!isOpen && 'md:opacity-0 md:hidden'}`}>
            {text}
        </span>
    </NavLink>
);
...
const HistoryIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>;
const PlaylistIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h7" /></svg>;
        ...
        >
            <div className="p-2 overflow-y-auto h-full scrollbar-thin scrollbar-thumb-gray-700 scrollbar-track-gray-900">
                <nav className="flex flex-col space-y-1">
                    ...
                    <div className={`border-t border-gray-700 my-2 ${!isOpen && 'md:mx-2'}`}></div>
                    
                    <h3 className={`px-4 pt-2 pb-1 text-xs text-gray-500 font-semibold uppercase transition-opacity duration-200 ${!isOpen && 'md:opacity-0 md:hidden'}`}>Categories</h3>
                    ...
                    {isAuthenticated && (
                        <>
                            <div className={`border-t border-gray-700 my-2 ${!isOpen && 'md:mx-2'}`}></div>
                            <h3 className={`px-4 pt-2 pb-1 text-xs text-gray-500 font-semibold uppercase transition-opacity duration-200 ${!isOpen && 'md:opacity-0 md:hidden'}`}>You</h3>
