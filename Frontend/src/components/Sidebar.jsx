...
const NavItem = ({ to, icon, text, end = false }) => (
    <NavLink 
        to={to} 
        end={to === "/"} // 'end' prop is only true for the homepage link
        className={({ isActive }) => 
...
function Sidebar({ isOpen }) {
    ...
    return (
        ...
        >
            <div className="p-4 overflow-y-auto h-full scrollbar-thin scrollbar-thumb-gray-700 scrollbar-track-gray-900">
                <nav className="flex flex-col space-y-2">
                    ...
                    {/* --- Authenticated User Links (Visible only if logged in) --- */}
                    {isAuthenticated && (
                        <>
                            <div className="border-t border-gray-700 my-4"></div>
